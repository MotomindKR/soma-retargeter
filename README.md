...
Bello descriptions and meshes are not redistributed by this project. Point the retargeter at an authorized `bello_full_body_viewer.xml` MJCF generated by the Bello asset pipeline. Bello uses the same single-stage Newton retargeting path as G1; only its `ik_map`, scaler, and foot-stabilizer configuration differ. The weights are tuned independently on this repository's bundled SOMA motion suite.
...
starts directly from its first frame because blending from the BVH zero pose
can steer the solver into a different limb branch. Enter `nix develop .#amass`
or run `uv sync --extra amass`, then point the converter at the separately
licensed SMPL-X body models:
...
To reproduce the bounded Bello parameter ablation, install the evaluation extra
and run the broad preset (240 frames per clip by default):
...
Use `--preset confirmation --max-frames 0` for the full 6,408-frame
confirmation suite.
